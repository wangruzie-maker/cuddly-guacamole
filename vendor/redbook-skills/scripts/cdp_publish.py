...
XHS_IMAGE_PUBLISH_URL = "https://creator.xiaohongshu.com/publish/imgNote?source=official"
...
class XiaohongshuPublisher:
    ...
    def _rect_from_content_quads(self, quads: list[list[float]] | None) -> dict[str, Any] | None:
        """Convert CDP content quads into a bounding rect."""
        if not quads or not quads[0] or len(quads[0]) < 8:
            return None
        xs = quads[0][0::2]
        ys = quads[0][1::2]
        x = min(xs)
    ...
    def _extract_red_button_backend_id(self, node: dict[str, Any]) -> int | None:
        """Find the publish button backend node id inside a pierced DOM tree."""
        attrs = node.get("attributes") or []
        attr_map = {attrs[i]: attrs[i + 1] for i in range(0, len(attrs), 2)}
        if node.get("nodeName") == "BUTTON" and "bg-red" in attr_map.get("class", ""):
            backend_id = node.get("backendNodeId")
            if backend_id:
                return int(backend_id)

        for child in node.get("children") or []:
            found = self._extract_red_button_backend_id(child)
            if found:
                return found

        for shadow_root in node.get("shadowRoots") or []:
            found = self._extract_red_button_backend_id(shadow_root)
            if found:
                return found

        return None

    def _find_publish_button_rect_via_pierce(self) -> dict[str, Any] | None:
        """Locate button.ce-btn.bg-red inside xhs-publish-btn via CDP shadow piercing."""
        try:
            self._send("DOM.enable")
        except CDPError:
            pass
        ...
        host_obj = self._send(
            "Runtime.evaluate",
            {
                "expression": f"document.querySelector({json.dumps(SELECTORS['publish_host'])})",
                "returnByValue": False,
            },
        )
        obj_id = host_obj.get("result", {}).get("objectId")
        if not obj_id:
            return None

        desc = self._send(
            "DOM.describeNode",
            {"objectId": obj_id, "depth": 5, "pierce": True},
        )
        backend_id = self._extract_red_button_backend_id(desc.get("node", {}))
        if not backend_id:
            return None

        quads_result = self._send(
            "DOM.getContentQuads",
            {"backendNodeId": backend_id},
        )
        rect = self._rect_from_content_quads(quads_result.get("quads"))
        if not rect:
            return None

        rect["source"] = f"{SELECTORS['publish_button_alt']}-pierce"
        return rect

    def _find_publish_button_rect_via_dom(self) -> dict[str, Any] | None:
        """Locate the publish button rect via page JS (legacy DOM and host fallback)."""
        publish_text = SELECTORS["publish_button_text"]
        selectors = [
        ...
        return self._evaluate(f"""
            (() => {{
                const publishText = {json.dumps(publish_text)};
                ...
                const isEnabled = (node) => {{
                    ...
                    if (String(node.className || "").includes("disabled")) {{
                        return false;
                    }}
                    return true;
                }};
                const toRect = (node, source) => {{
                    const rect = node.getBoundingClientRect();
                    return {{
                        x: rect.x,
                        y: rect.y,
                ...
                const walk = (root) => {{
                    for (const selector of selectors) {{
                        ...
                        if (visible(button) && isEnabled(button)) {{
                            return toRect(button, selector);
                        }}
                    }}
                    ...
                    for (const node of buttons) {{
                        if (!visible(node) || !isEnabled(node)) {{
                            continue;
                        }}
                        const text = (node.innerText || node.textContent || "").trim();
                        if (text === publishText) {{
                            return toRect(node, "text-match");
                        }}
                    }}
                    return null;
                }};

                const direct = walk(document);
                if (direct) {{
                    return direct;
                }}

                const host = document.querySelector(
                    {json.dumps(SELECTORS["publish_host"])} + '[is-publish="true"], '
                    + {json.dumps(SELECTORS["publish_host"])}
                );
                if (!host) {{
                    return null;
                }}
                const submitDisabled = String(
                ...
                const publishHeight = 40;
                const publishX = hostRect.x + hostRect.width - publishWidth - 16;
                const publishY = hostRect.y + (hostRect.height - publishHeight) / 2;
                return {{
                    x: publishX,
                    y: publishY,
                    width: publishWidth,
                    height: publishHeight,
                    source: "xhs-publish-btn-host",
                }};
            }})()
        """)

    def _find_publish_button_rect(self) -> dict[str, Any] | None:
        """Locate the publish button using pierced shadow DOM first, then DOM JS."""
        rect = self._find_publish_button_rect_via_pierce()
        if not rect:
            rect = self._find_publish_button_rect_via_dom()
        if isinstance(rect, dict) and rect.get("source"):
            print(f"[cdp_publish] Publish button located via {rect.get('source')}.")
        return rect

    def _get_publish_button_rect(self) -> dict[str, Any] | None:
        """Locate the current publish button using current and legacy selectors."""
        return self._find_publish_button_rect()

    def _verify_publish_success(self, title_hint: str | None = None) -> bool:
        """Best-effort verification that the publish action actually completed."""
        self._sleep(4, minimum_seconds=2.0)
        state = self._evaluate("""
            (() => {
                ...
                const hasPreview = !!document.querySelector(".img-preview-area .pr");
                const publishHost = document.querySelector("xhs-publish-btn");
                return {
                    url,
                    title,
                    hasPreview,
                    hasPublishHost: !!publishHost,
                    hasSuccessText: /发布成功|提交成功|笔记已发布|进入审核|发布完成/.test(text),
                    stillOnPublishEditor:
                        url.includes("/publish/publish") && hasPreview && !!publishHost,
                };
            })()
        """)
        if not isinstance(state, dict):
            return False
        if state.get("hasSuccessText"):
            return True
        if not state.get("stillOnPublishEditor"):
            return True
        if title_hint and state.get("title") != title_hint:
            return True
        return False

    def _is_publish_button_ready(self) -> bool:
        """Return True when the publish button is present, visible and not disabled."""
        return bool(self._find_publish_button_rect())
    ...
    def _upload_input_ready(self) -> bool:
        return bool(
            self._evaluate(
    ...
    def _ensure_image_publish_mode(self):
        """Enter image+text publish mode via direct URL or tab click."""
        if self._upload_input_ready():
            print("[cdp_publish] Already on image publish page.")
            return

        print("[cdp_publish] Opening image publish page directly...")
        self._navigate(XHS_IMAGE_PUBLISH_URL)
        self._sleep(2.5, minimum_seconds=1.2)
        if self._upload_input_ready():
            print("[cdp_publish] Image publish page ready (imgNote).")
            return

        print("[cdp_publish] imgNote route unavailable, falling back to publish tab...")
    ...
    def _click_tab(self, tab_selector: str, tab_text: str):
        ...
        clicked = self._evaluate(f"""
            (function() {{
                ...
                function isVisible(el) {{
                    ...
                    if (tabindex === '-1' && !/\\bactive\\b/.test(className)) return false;
                    return true;
                }}
        ...
        if not clicked:
            if "图文" in tab_text:
                ...
                tab_labels = self._evaluate("""
                    Array.from(document.querySelectorAll('div.creator-tab, [role="tab"], button'))
    ...
    def _click_publish(self, scheduled: bool = False):
        ...
        print(
            f"[cdp_publish] Clicking publish button at ({cx:.0f}, {cy:.0f}) "
            f"via {source}..."
        )
        self._click_mouse(cx, cy)
        print("[cdp_publish] Publish button clicked.")

        if not self._verify_publish_success():
            raise CDPError(
                "Publish button was clicked, but the note still appears to be in the editor. "
                "Please confirm manually in the browser."
            )
    ...
    def publish(
        ...
        # Step 1: Enter image publish mode (direct URL preferred)
        self._ensure_image_publish_mode()

        # Step 2: Upload images (editor appears after upload)
        ...
        # Step 5: Set schedule publish time (if provided)
