...
metadata:
  ...
  install-path: ~/.cursor/skills/redbook-skills
  patched: 2026-06-12-macOS-publish-button
---
...
本技能为**全局 Cursor Agent Skill**，代码与文档统一存放在：

| 用途 | 路径 |
|------|------|
| **技能根目录** | `/Users/chaos/.cursor/skills/redbook-skills/` |
| **技能主文件** | `/Users/chaos/.cursor/skills/redbook-skills/SKILL.md` |
| **发布脚本** | `/Users/chaos/.cursor/skills/redbook-skills/scripts/publish_pipeline.py` |
| **CDP 核心** | `/Users/chaos/.cursor/skills/redbook-skills/scripts/cdp_publish.py` |
| **Chrome 启动** | `/Users/chaos/.cursor/skills/redbook-skills/scripts/chrome_launcher.py` |

在 Cursor 中查找方式：

1. **Agent 对话**输入 `/redbook-skills` 手动触发
2. **Settings → Rules → Agent Decides** 中查看 `redbook-skills`
3. 对话中说「发布到小红书 / 发小红书图文」等，Agent 会自动匹配本技能

环境要求：`Python 3.10+`、Google Chrome、依赖见 `requirements.txt`（`requests`、`websockets`）。

## 2026 创作者中心发布按钮适配（macOS 已验证）

小红书 2026 版发布页将「发布 / 暂存离开」放入 Web Component `<xhs-publish-btn>` 的 **Shadow DOM**，真实按钮为 `button.ce-btn.bg-red`。

本技能已在 `scripts/cdp_publish.py` 固化以下逻辑：

1. **优先**用 CDP `DOM.describeNode(pierce=true)` + `DOM.getContentQuads` 精确定位 `button.ce-btn.bg-red`
2. **兜底**旧选择器 `.publish-page-publish-btn button.bg-red` 与 `xhs-publish-btn` 坐标估算
3. **发布后验证**：若仍停留在编辑页（图片预览 + 发布组件仍在），判定失败并报错，避免误报 `PUBLISHED`
4. **可选复核**：`content-data` 查创作者笔记列表确认新帖

**禁止**在未点击到真实按钮时打印 `PUBLISH_STATUS: PUBLISHED`。

## 一键图文发布（推荐命令）

执行前必须让用户确认标题、正文、图片绝对路径。技能根目录变量：

```bash
SKILL_ROOT="/Users/chaos/.cursor/skills/redbook-skills"
```

```bash
# 1) 首次或过期时登录
python3 "$SKILL_ROOT/scripts/cdp_publish.py" login

# 2) 检查登录
python3 "$SKILL_ROOT/scripts/cdp_publish.py" check-login

# 3) 图文发布（填表 + 点击发布 + 验证）
python3 "$SKILL_ROOT/scripts/publish_pipeline.py" \
  --title-file /tmp/xhs-publish/title.txt \
  --content-file /tmp/xhs-publish/content.txt \
  --images "/abs/path/to/image.png"

# 4) 发布后复核（查最近笔记）
python3 "$SKILL_ROOT/scripts/cdp_publish.py" content-data --page-num 1 --page-size 5
```

仅预览不点发布：在上述命令末尾加 `--preview`。

## 多账号批量发布（串行）

技能**不支持** 80 账号并发；推荐 **一个 Chrome、一个账号、发完再切下一个**。
...
# 批量添加账号名（示例）
python3 account_manager.py add acc01 --alias "账号1"
python3 account_manager.py add acc02 --alias "账号2"

# 每个账号首次需扫码登录（有窗口）
python3 cdp_publish.py --account acc01 login
python3 cdp_publish.py --account acc02 login

python3 cdp_publish.py list-accounts
```

账号配置：`$SKILL_ROOT/config/accounts.json`  
Profile 目录：`~/Google/Chrome/XiaohongshuProfiles/<账号名>/`

### 第二步：准备批量清单 manifest

复制示例并填写**每号不同**的标题、正文、图片（绝对路径）：

`$SKILL_ROOT/config/batch_manifest.example.json`

### 第三步：串行批量发布

```bash
python3 "$SKILL_ROOT/scripts/batch_publish.py" \
  --manifest "$SKILL_ROOT/config/my_batch.json" \
  --default-delay 300

# 只预览计划
python3 "$SKILL_ROOT/scripts/batch_publish.py" \
  --manifest "$SKILL_ROOT/config/my_batch.json" --dry-run

# 从中途某个账号继续
python3 "$SKILL_ROOT/scripts/batch_publish.py" \
  --manifest "$SKILL_ROOT/config/my_batch.json" --start-from acc05
```

脚本行为：按清单顺序 → 重启 Chrome（切 Profile）→ 检查登录 → `publish_pipeline` → `content-data` 复核 → 等待间隔 → 下一个账号。  
结果摘要：`/tmp/xhs-batch/batch_summary_*.json`

### 批量操作建议

- **串行，不要并发**：`publish_pipeline` 有单实例锁，同一台机器一次只跑一个发布流程最稳。
- **每号内容必须差异化**：标题、正文、图片不要 80 号完全相同。
- **拉长间隔**：`delay_after_seconds` 建议 ≥300 秒，量大时更长； `--timing-jitter 0.35` 仅作轻微延迟抖动。
- **先小批量试跑**：例如 3 个测试号验证 manifest 与登录，再扩大。
- **失败续跑**：用 `--start-from 失败账号名` 从断点继续。
- **无法保证不封号**：批量越大、内容越同质、频率越高，风控风险越高；重要账号保留人工终审。
...
- 发布按钮新版 DOM：`<xhs-publish-btn>` 内 Shadow DOM 的 `button.ce-btn.bg-red`；勿仅用旧选择器 `.publish-page-publish-btn button.bg-red`。
- 点击发布后必须调用 `_verify_publish_success()`；失败时提示用户到 Chrome 手动点「发布」，并用 `content-data` 复核。
...
- 页面选择器失效：提示检查 `scripts/cdp_publish.py` 中选择器并更新；2026 版发布按钮见上文「2026 创作者中心发布按钮适配」。
- 脚本显示 `PUBLISH_STATUS: PUBLISHED` 但账号无新帖：用 `content-data` 复核；若仍无，说明点击未生效，改用 CDP pierce 路径或请用户手动发布。
